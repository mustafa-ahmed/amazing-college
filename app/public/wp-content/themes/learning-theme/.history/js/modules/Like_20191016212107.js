...
class Like {
    ...
    ourClickDispatcher(e) {
        ...
        debugger;
        if (currentLikeBox[0].attr['data-exists'] == 'yes') {
    ...
    createLike(currentLikeBox) {
        $.ajax({
            ...
            'success': (response) => {
                currentLikeBox[0].attr('data-exists', 'yes');
                var likeCount = parseInt(currentLikeBox.find(".like-count").html(), 10);
                likeCount++;
                currentLikeBox.find(".like-count").html(likeCount);
                currentLikeBox[0].attr('data-like', response);
    ...
    deleteLike(currentLikeBox) {
        $.ajax({
            ...
            'type': 'DELETE',
            'success': (response) => {
                currentLikeBox[0].attr('data-exists', 'no');
                var likeCount = parseInt(currentLikeBox.find(".like-count").html(), 10);
                likeCount--;
                currentLikeBox.find(".like-count").html(likeCount);
                currentLikeBox[0].attr('data-like', '');
                console.log(response);
            },
            'error': (err) => {
                alert(err);
