...
class Search {
    ...
    getResults() {
        ...
        // delete this code after use it as reference
        $.when(
            $.getJSON(globalObject.root_url + '/wp-json/wp/v2/posts?search=' + this.searchField.val()),
            $.getJSON(globalObject.root_url + '/wp-json/wp/v2/pages?search=' + this.searchField.val()),
        ).then((posts, pages) => {
            var results = posts[0].concat(pages[0]);
            this.resultsDiv.html(`
            <h2 class="search-overlay__section-title">General Information</h2>
            ${(results.length) ? `<ul class="link-list mi-list">
            ${results.map(item => {
                return `
                    <li>
